...
The hands-on exercise for this session focuses on word embedding and provides an overview of the data structures, and functions relevant for, estimating word vectors for word-embedding analyses.
...
You can also download the tweets sample from the download links on the README.
...
We will then use a technique called the "Singular Value Decomposition" (SVD). This is a dimensionality reduction technique where the first axis of the resulting composition is designed to capture the most variance, the second the second-most etc...
...
Well, we're first splitting up ("unnesting") our text into a series of words or "ngrams" of length 6,

In other words, for postID `n` with words `k` indexed by `i`, we take words `i_1` ... `i_6`, then we take words `i_2` ... `i_7`. Try just running the first two lines of the full code as below to see what this means in practice.
...
tidy_skipgrams$ngram[1:5]
```

We see here that each tweet (if it is more than 6 words) is split up into individual ngrams of length 6. 

Now we add in some further steps. 
...
To load the pre-estimated skipgrams instead of estimating yourself, you can download it from the download links on the README.
...
Well, for each tweet, and each round of six words that appear in that tweet, we have an individual "skipgramID."

You'll see in just a moment why this is useful. 

What next?

Well we can now calculate a set of probabilities from our skipgrams. We do so with the `pairwise_count()` function from the <tt>widyr</tt> package. 

Essentially, this function is saying: for each skipgramID count the number of times a word appears with another word for that feature (where the feature is the skipgramID). We set `diag` to `TRUE` when we also want to count the number of times a word appears near itself.

You can imagine this as follows: say you were reading a chapter of a book. But instead of reading it all in one go. You read 6 word sections and start again each time with the second word from the last 6 word sequence you read. That is what the computer is doing to count up co-occurrences within each six-word window. 
...
So we see, for example, the words us and the appear 4084 times together. Denominating that by the total n of word pairings (or `sum(skipgram_probs$n)`), gives us our probability p.
...
And then we calculate the joint probability as the skipgram probability divided by the unigram probability for the first word in the pairing divided by the unigram probability for the second word in the pairing. This is equivalent to: 
...
Note that this procedure may take a long while. 
...
To load a pre-estimated SVD from the PMI matrix you created, you download it from the download links on the README.
...
You can also download the GloVe embedding layer from the download links on the README.
